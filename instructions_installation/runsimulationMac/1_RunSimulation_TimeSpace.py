...
def startSumo(acc=False):
    ...
    if acc:
        sumoConfigFile = "SumoSimulation/Configuration_ACC.sumocfg" #"Configuration.sumocfg"
...
def runSimulation(x=5, acc=False):
    # Start Sumo
    startSumo(acc)
...
plt.figure("Raum-Zeit-Diagramm", figsize=(12,10))
...
vehicle_t, vehicle_s, vehicle_v = runSimulation(x=10, acc=True)
plt.subplot(2,2,3)
plt.title("Raum-Zeit-Diagramm (10 Fahrzeuge + ACC)")
plt.ylabel("Position [m]")
plt.xlabel("Zeit [s]")
plt.ylim(y_range2)
plt.xlim(x_range)
